...
onMounted(() => {
  ...
  customInsert = () => {
    ...
    insertHard(store.customWidgets[widgetName.value].html || store.customWidgets[widgetName.value], `<<${widgetName.value}>>`);
  }

  customDelete = () => {
    if (widgetName.value === 'new') return;
    delete store.customWidgets[widgetName.value];
    widgetName.value = savedNames.value[0] || 'new'
    localStorage.setItem('customWidgets', JSON.stringify(store.customWidgets));
  }

  customExport = () => {
    let twee = `<!-- Generated by DOL-pancake -->\n:: Widget ${Date.now()} [widget]\n`;

    Object.entries(store.customWidgets).forEach(([name, widget]) => {
      twee += `\n<<widget "${name}">>\n${
  ...
  editingSave = () => {
    ...
    if (store.customWidgets[editingName.value]) {
    ...
    store.customWidgets[editingName.value] = {
